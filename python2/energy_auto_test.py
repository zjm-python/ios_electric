...
from xtencentmeetingtest.bvt_case.IosTest import IosTest
from xtencentmeetingtest.bvt_case.IosTest import switch_ui_test
...
def wait_full_charged(uuid, full_level):
    while True:
        level, is_full = get_battery_info(uuid)
        if is_full or level >= full_level:
            break
        time.sleep(5)
    return True
...
def energy_auto_test(ip, port, usb_port, uuid, bundle_id):
    # 连接数据线，充电
    print('step 1: start connect data line and charged')
    usb_ctrl_client.usb_ctrl_client(ip, port, usb_port, 1)

    # 充满电
    print('get charged state')
    wait_full_charged(uuid, 98)
    ...
    # 配置app参数
    settings.IOS_APP_BUNDLE_ID = bundle_id
    settings.PKG_ROOT = None  # 测试包存放的目录 预留用于自定义安装包
    do_real_test()
...
def energy_auto_test_cycle(ip, port, usb_port, uuid, output_dir, bundle_id, app_name, times):
    start_time = 0
    for i in range(times):
        start_time = energy_auto_test(ip, port, usb_port, uuid, bundle_id)
    # 等待同步和解析
...
def restart_device(uuid):
    print('restart device')
    sync_cmd = r'idevicediagnostics'  # 这里r可以可以不管空格和中文字符的烦恼
    child = subprocess.Popen(sync_cmd + ' -u ' + uuid, shell=True)
    return_code = child.wait()
    print('restart result ', return_code)
    time.sleep(20)


def energy_auto_test_with_config():
    energy_auto_test_cycle(ip=c.usb_server_ip, port=c.usb_server_port, usb_port=c.usb_child_port,
                           uuid=settings.IOS_HOST, output_dir=c.output_dir,
                           bundle_id=settings.IOS_APP_BUNDLE_ID, app_name=c.app_name, times=c.cycle_times)


def sync_and_parse_with_config():
    sync_and_parse(uuid=settings.IOS_HOST, output_dir=c.output_dir, bundle_id=settings.IOS_APP_BUNDLE_ID, app_name=c.app_name)
